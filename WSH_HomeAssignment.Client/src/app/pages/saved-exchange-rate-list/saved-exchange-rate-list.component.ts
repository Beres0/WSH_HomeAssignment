...
import { PaginationArgs, PaginationDto, SavedExchangeRateDto } from 'src/app/api-proxy-services/models/types';
...
export class SavedExchangeRateListComponent {
  ...
  constructor(private service:ExchangeRatesService,validations:ValidationStoreService){
  ...
  confirm(e:SavedExchangeRateDto){
    ...
    this.service.updateSaved(e.date,e.currency,{note:note}).subscribe({error:(e)=>{
      console.log(e)
      window.location.reload()
    }})
  }
  delete(e:SavedExchangeRateDto){
      this.service.deleteSaved(e.date,e.currency).subscribe({next:(r)=>{
        const index=this.items.findIndex(value=>{
          return e.date.year==value.date.year&&e.date.month==value.date.month&&e.date.day==value.date.day && e.currency==value.currency
        })
        if(index!=undefined){
