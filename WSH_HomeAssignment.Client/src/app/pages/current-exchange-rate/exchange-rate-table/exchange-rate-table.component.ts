...
export class ExchangeRateTableComponent {
    ...
    hasNote(currency: string) {
        return (
            this.savedCurrent?.notes != undefined &&
            (this.savedCurrent.notes as any)[currency] != undefined
        );
    }
    ...
    savedContains(currency: string) {
        return (
            ...
            (this.savedCurrent?.exchangeRates as any)[currency] != undefined
        );
    }
    ...
    async save(exchangeRate: ExchangeRateDto, button: HTMLButtonElement) {
        try {
            ...
            this.set(
                this.savedCurrent?.exchangeRates,
                exchangeRate.currency,
                exchangeRate
            );
            this.set(
                this.savedCurrent?.notes,
                exchangeRate.currency,
                undefined
            );
        } catch (error) {
