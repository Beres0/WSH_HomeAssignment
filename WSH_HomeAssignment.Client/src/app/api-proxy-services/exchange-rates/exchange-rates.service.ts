...
export class ExchangeRatesService extends ApiService {
    ...
    getSavedCurrent() {
        return this.http.get<SavedDailyExchangeRatesDto>(
            this.url + '/saved/current'
        );
    }
    getSavedList(args?: PaginationArgs) {
        return this.http.get<PaginationDto<SavedExchangeRateDto>>(
            this.url + '/saved',
    ...
    getSaved(date: DateDto, currency: string) {
        return this.http.get<SavedExchangeRateDto>(
            this.getSavedFullRoute(date, currency)
        );
    }
